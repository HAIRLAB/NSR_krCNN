...
def segmentation1(data):
    ...
    for item in data:
        ...
        temp.append(a)
        train_input.append(a[:int(len(a)*0.7),:])
        test_input.append(a[int(len(a)*0.7):,:])
    output=train_input[0]
    for item in train_input[1:]:
        output=np.concatenate((output,item),axis=0)
    output2=test_input[0]
    for item in test_input[1:]:
        output2=np.concatenate((output2,item),axis=0)
    train_input=output
    test_input=output2
...
train_input1,test_input1=segmentation1(data1_vib)
train_input2,test_input2=segmentation2(data2_vib)
train_input3,test_input3=segmentation1(data3_vib)
train_input4,test_input4=segmentation1(data4_vib)
